...
1. Run `supabase/cart-orders.sql` first so the base account, order, and inquiry tables exist. Re-run it after pulling order-flow updates because it also carries the admin order review policies.
...
- The admin panel can review orders, change order status, and manage products once `is_admin = true` is set on your profile.
