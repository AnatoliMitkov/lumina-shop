        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.04fr_0.96fr] gap-10 md:gap-12 items-start">
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                ...
                <section className="border border-[#1C1C1C]/10 bg-white/58 rounded-sm p-6 md:p-8 flex flex-col gap-6">
                                ...
                                <PricingValidationCard
                                    ...
                                    message={pricingPreview.affiliate.status === 'applied'
                                        ...
                                        : pricingPreview.affiliate.status === 'tracked'
                                            ? `${pricingPreview.affiliate.code} ${getText('checkout.codes.affiliate_result.tracked', localizedFallback('tracks the partner attribution with no shopper discount.', 'отчита партньорската атрибуция без отстъпка за клиента.'))}`
                    ...
                    <div className="border border-[#1C1C1C]/10 bg-[#EFECE8] rounded-sm px-4 py-4 text-sm leading-relaxed text-[#1C1C1C]/62">
                        <EditableText contentKey="checkout.codes.notice" fallback={localizedFallback('Codes are validated against the live studio settings. Shopper savings update immediately, and delivery coverage follows the current studio rules before payment opens.', 'Кодовете се валидират спрямо текущите студийни настройки. Спестяването за клиента се обновява веднага, а обхватът на доставката следва активните правила преди да се отвори плащането.')} editorLabel="Checkout codes notice" />
                    </div>
