...
import { fetchPromotionRecords } from '../promotions';
import { sendOrderPaymentConfirmation } from '../order-mail';
...
export async function getOrderForSession(supabase, sessionId) {
...
export async function syncCompletedSession(supabase, stripeSession) {
  ...
  try {
    ...
    // Payment confirmation email is best-effort so payment finalization can still succeed.
  }

  return confirmedOrder;
}

export async function syncSessionState(supabase, stripeSession, nextStatus) {
  const { paymentRecord, order } = await getOrderForSession(supabase, stripeSession.id);
